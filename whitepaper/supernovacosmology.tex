...
Since the described steps are all necessary for deter, a metric for supernovae cosmology must quantify the ability to perform these steps on each supernova of the sample. To connect this to the output of OpSims, we employ the following strategy:
\begin{itemize}
    \item we study the sequences of observations in small spatial regions of the sky so that the sequences of observations relate to positions of astrophysical objects like supernovae. In Fig., we show such a sequence for a DDF and SFD field for the entire survey, and for a single year.
    \item On each such spatial region, we look at sliding time windows, each time window of size about 70 days (corresponding roughly to a supernova Type Ia  lifetime starting 20 days before peak and extending to about 50 days after peak). We call this a \textbf{perSNMetric} $PM$. We assign a value to each of these time-windows meant to estimate the quality of observations for a supernova whose rough lifetime matches that time window. The values assigned to each time-window defines our metric, and the
 values must be assigned to quantify the success of the steps mentioned above.
