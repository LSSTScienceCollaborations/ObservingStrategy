...
\new{Within sims\_maf, metrics A (parallax factor distribution) and B
...
\new{{\it Parallax factor coverage:} This is {\tt
    calibrationMetrics.ParallaxCoverageMetric} in sims\_maf. It
  returns the inverse variance-weighted mean angular separation
  $\langle r \rangle$~of a star observed at the predicted date, from
  the guiding center of the path it traces over the course of a year
  due to parallax. This mean radius is scaled to the maximum angular
  separation around this annual path, and variances used are the
  estimates of (uncrowded) astrometric precision returned by OpSim for
  a star of specified fiducial magnitude at the center of the HEALPIX
  of interest. What constitutes a ``good'' value depends on the
  location of the star in ecliptic co-ordinates; near either ecliptic
  pole a star with uniform parallax coverage would have $\langle r
  \rangle \approx 1.0$~while on the ecliptic uniform coverage would
  produce $\langle r \rangle \approx 0.5$.}

\new{{\it Parallax-Hour angle correlation:} This is metric {\tt
  ...
  correlation coefficient $\rho$~between the best-fit parallax and
  DCR, returning values in the range $-1.0 \le \rho \le +1.0$. The
  range of acceptable values for this metric is still under
  investigation; Monte Carlo simulation by one of us (DGM) suggests
  the parallax error becomes independent of $\rho$~(i.e. other effects
  dominate) for values $|\rho| \lesssim 0.7$.}
