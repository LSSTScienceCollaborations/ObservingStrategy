...
\new{ \item[A1:] In principle yes (in that we do not yet know what
  combination of depth and volume would make the sample of stars in
  the Solar Neighborhood the most complete, for example). However we
  do not yet have the FoM evaluation to set quantitative constraints.}
...
\new{ \item[A3:] More visits at the standard exposure time are
  ...
  to achieve as broad a temporal coverage as possible (see Section \ref{sec:MW_Astrometry:MW_Astrometry_metrics}).}
...
\new{ \item[A4:] Not for the example of detecting Galactic Halo
  ...
  \ref{sec:MW_Astrometry:MW_Astrometry_measurements}). Converseley,
  avoiding the inner Galactic mid-plane would reduce the completeness
  of the census of the Solar Neighborhood populations due to the
  reduction in total area surveyed. However, this reduction may be
  incremental rather than serious. The impact of an inner-plane zone
  of avoidance on the recovery of the parameters describing these
  constituent populations has not yet been evaluated.}
...
\new{ \item[A5:] Yes, but indirectly through the requirement to
  ...
  observations in $u$~and $Y$ filters separately, as well as at least one
  mid-range filter. However the quantitative impact on population
...
\new{ \item[A7:] The same filter is weakly preferred, to allow some accounting for changes in local conditions between the two visits.}
...
\new{ \item[A8:] Possibly a single field observed with observations
  clustered into the first year to sample the parallactic ellipse as
  fully as possible. However, because this would likely sacrifice
  future time coverage of this field for the following nine years of
  the survey, such a study might more properly be performed with
...
\new{ \item[A10:] Not as long as the achieved limiting depth is accurately known after the observation takes place.}
