...
The accelerated expansion of the Universe is one of the 
most exciting and fundamental discoveries in this
century \citep{Riess1998,Perlmutter1999}.
This discovery was made using Type Ia supernovae (SNe) as cosmological candles, and
...
defined in \citet{Lochner2016}, which is a good balance between purity and completeness. The 
probability to determine if SNe are Ia or other types is low, since the light curves from WFD have 
poor qualities.
We have hoped to apply SN classification metric to the opsim light curves, but %individual light 
curves are uncertain. 
%more effort is required to be able to determine if the light curves are sufficient to determine the 
types of SNe.
since SN classification is a challenging task only using photometric data without follow-up 
spectroscopy, further 
studied are required to estimate the probability. 
...
\begin{itemize}
\item A high portion of the light curves can be identified as transients using  a SN based on SN 
...
\item We have explored ways to improve SN classification \citep{Lochner2016} and the classification metric is defined in Section 
...
determined for LSST. This is discussed in \autoref{sec:persnmetric}
...
The DDF are useful for SNe at redshifts between 0.6 and 1.2, the WFD will help us to increase 
...
Since the cosmology measurement requires low redshift distance moduli as well as high redshift distance moduli, a large sample 
of low redshift SN will boost the cosmological parameter inference as well as providing a 
useful sample of nearby supernovae to better constrain variations in SNe populations (e.g. core-collapse SNe). 
Light curves from WFD
with reasonable quality can be achieved using our proposed
observing strategy.

A larger sample of well-characterized light curves from the WFD can address two key science 
goals:
\begin{itemize}
\item To tightly constrain cosmological parameters ($\Omega_M$ and $\Omega_\Lambda$) 
...
\item The WFD offers a unique opportunity for isotropy (complementary to large-scale galaxy 
...
\hline
 FieldID & (R.A., Decl.) & \# of LSST visits per year (u,g,r,i,z,y)       & \# of Avg. data per SN period per filter & SNDM  & SNQM\\
\hline
...
\hline
\end{tabular}
\caption{Table of 15 fields in the OpSim \texttt{enigma\_1189}. The first column is simply an 
...
SNQM are SN discovery and quality metric, respectively.}
...
Our primary goal is to  achieve our goal of 7-9 points per light 
...
We propose to a new observing strategy that provides a dense sampling in time.
This cadence is to improve observing strategy of WFD in order to produce SN light curves which are 
...
to be enhanced by a factor of 3 (i.e.$\times$3), hereafter. 
...
The enhanced light curve is generated by pseudo-random numbers with uniform distributions in time.
...
toward RA. of 58$^{\circ}$ and Decl. of -27$^{\circ}$ (left) and the same light curve
...
We define the 10,000 deg$^2$ survey area, which is the visible sky for a given time of the 
...
sky  low air-mass, but at the same time 
