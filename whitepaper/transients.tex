...
%We discuss several science-specific metrics in the sections that follow.
%Here we describe several \textit{generic} metrics valuable for considering
%transient performance in general:

%\subsubsection{Observation Gap Metrics (\texttt{intraNightGapMetric},
%\texttt{interNightGapMetric})}
%
%Calculates the median time between consecutive observations of a field in a
%single night and in more than one night.  SQL queries can be used to
%restrict to single-filter revisits.

%\subsubsection{Transient Metric (\texttt{transientMetric},
%\texttt{transientAsciiMetric})}
%
%Calculate what fraction of transients would be detected using either a
%sawtooth lightcurve or a user-specified ASCII
%input file for the lightcurve.
...
% FBB: moved back here after discussion on what is the threshold for being pulled out of Future Work
