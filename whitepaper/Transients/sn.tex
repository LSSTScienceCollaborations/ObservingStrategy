...
color. Colors of course allow to learn a lot more about the
...
SNe satisfying proper quality criteria to enable to distinguish
...
with standard deviation $\sigma_\mathrm{pop} = 0.5$, 1, and 2.  We
...
we report the KS or AD test's (whichever performs best) ability to
...
that for RG progenitor the detectability is enhanced by $\sim50\%$ in
 $g'$ and slightly less in $r'$.
...
\frac{1.0}{2.0~\sigma_\mathrm{pop}/\sqrt{2}}$.

With the goal of distinguishing a SD contribution of 20\% to the SN Ia
population, we need more than 1000 detections, in 2 filters within 1
day, and a $\sigma_\mathrm{pop}<1.0$ (\autoref{tab:SNprogenitors}. But
the pairs of observations we recovered at the previous step are within
the first 10 days but with any gap in time. \emph{Second shortcut}: To
include the constraint that the detections should be within 24 hours
we refer to the \texttt{InterNightGapsMetric}, which is plotted in
...
which is a \emph{conservative} assumption our numbers drop by a
factor 10.
...
of SNLS data~\citep{Bianco11} with \opsimdbref{db:baseCadence}! Meanwhile 3 month of survey fall short of the goal for 
