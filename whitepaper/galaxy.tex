...
science no matter the spatial crowding in a given science case, which
we encourage the reader to incorporate into their thinking when
considering possible metrics. [Language will change as metrics are
  produced.]
...
science cases. These naturally divide into two classes: (i) science
cases mostly or entirely in the Plane - for which metrics will be
critical in determining not just the as-yet-unfinalized observing
strategy, but whether a given science case can be done at all given a
realistic allocation - and (ii). cases that fall within WFD, for which
metrics are still needed to assess the tradeoff between different
options in WFD itself. The sections that follow are:
...
{\it Milky Way science cases mostly in the Plane:}

Section \ref{sec:MW_spatial_structure}: Spatial Structure of the Milky Way Galaxy, including the plane and bulge;
