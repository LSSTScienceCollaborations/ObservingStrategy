...
\href{http://ops2.lsst.org:8888}{simulated surveys} is presented here,
...
output of such a simulation is an “observation history”, which is a record of times, pointings and a record of associated
environmental  data  and  telescope  activities  throughout  the  simulated  survey.  This history can be examined to assess
...
\begin{itemize}
\item {\bf Wide, Fast, Deep (WFD):} The WFD (also called the Universal Cadence) is the primary LSST survey, taking 85-95\% of the observing time and covering 18,000 square degrees of sky. This observing proposal is usually configured to attempt making observing pairs spaced $\sim40$\ minutes apart.  This temporal spacing is designed to optimize the detection of moving solar system objects. This proposal typically balances the six $ugrizy$ filters, observing each field every $\sim3$\ days.
\item {\bf North Ecliptic Spur (NES):} The NES is an extension to reach the Ecliptic at higher airmass than the WFD survey typically covers.  The NES typically does not include the $u$-filter.
\item {\bf Galactic Plane:} This proposal covers the region where LSST is expected to be highly confused by the density of stellar sources.  Typically takes fewer total exposures per field than the WFD survey.
\item {\bf South Celestial Pole (SCP):} The SCP is an extension to higher airmass than the WFD to cover the region south of declination -60 degrees.  This proposal includes $ugrizy$, but takes fewer exposures per field than the WFD.
...
demonstrate that this hypothethical ``survey reserve'', relative to the Universal
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\opsimdb[db:DoubleUbandExptime]{kraken\_1045}{Baseline Cadence, but with 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\opsimdb[db:DoubleUbandExptimewithNESpur]{kraken\_1059}{Baseline Cadence, 
but with doubled $u$-band exp.\ time and the same number of visits.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
limit from 1.5 to 1.3? To avoid complicated analysis, we use only the Universal
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\opsimdb[db:NormalGalacticPlane]{astro\_lsst\_01\_1004}{Extend Universal
...
this to the Baseline Cadence, \opsimdbref{db:baseCadence}.
 ...
 {\bf Analysis Results:}  This simulated cadence is named
 \opsimdbref{db:NormalGalacticPlane}. Compared with the
