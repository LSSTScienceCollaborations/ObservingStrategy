...
\item[A1:] The main FoM for AGN science is maximizing the number of
AGNs detected. Since the co-added depth is already pushing well into
the regime of low-luminosity AGNs, that suggests that area is
preferred over depth.  AGNs would benefit from increasing the area
beyond 18,000 deg$^2$ as long as we are more than 18 degrees from the
Galactic Plane (Richards et al. 2009).  Moreover, should there be any
downscope, the AGN SC might prefer to sacrifice WFD depth, except for
the Y band, to maintain its area.
...
coverage would be detrimental to AGN science.  Areas within 18
degrees of the Galactic Plane will result in significant contamination
of the AGN sample in those regions.

\item[Q6:] {\it Does the science case place any constraints on the
cadence for deep drilling fields?}

\item[A6:] Nearly any cadence discussed for the deep drilling fields
will be more than adequate for AGN selection; however added depth
during commissioning would enable more robust truth tables.  AGN
physics, on the other hand, would benefit from fast time sampling
according to analysis of Kepler data.

\item[Q7:] {\it Assuming two visits per night, would the science case
benefit if they are obtained in the same band or not?}

\item[A7:] There is no obvious preference here; however there may be
some benefit to merging of light curves from the different bandpasses
if the same band is observed more than once in a night (to help
distinguish real variability trends from photometric error).  We are
in the process of examining this in more detail.  However, the AGN
*science cases* may prefer different bands.
...
\item[A8:] AGNs will benefit from enchanced cadence in the DDFs.
Just about any cadence under discussion would suffice, so AGNs may not
be a driver here.  However, AGNs may benefit from a faster cadence in
the WFD area ---so long as the time baselines are distributed as
smoothly as possible in Delta time (i.e., roughly equal number of
observations separated by 1 day, 1 month, 1 year) so as to more
uniformly sample the "structure function" (or the power spectral
density).  Such a distribution is difficult to achieve due to seasonal
gaps without pushing observations to high airmass.  Repeat periods of
high cadence in a region would not be desirable for AGNs since that
would result in an overdensity in Delta time space.  Both the AGN
census and AGN science might benefit from a *twilight*-only rolling
cadence in 2-3 bands (one of which should be g) with 5s exposures and
a looser airmass limit.

\item[Q9:] {\it Does the science case place any constraints on the
sampling of observing conditions (e.g., seeing, dark sky, airmass),
possibly as a function of band, etc.?}

\item[A9:] AGNs impose no formal {\em constraint} here; however, the
selection and photometric redshift estimation of AGNs will benefit
from some high airmass observations in the u- and g-bands due to the
Differential Chromatic Refraction (DCR) effect (Kaczmarczik et
al. 2009; Peters et al. 2015).  We are currently attempting to create
a MAF that will quantify this for the WFD area.  In the DDFs the
airmass distribution will naturally extend to high airmass and can be
used to empirically quantify the gain enabled by high airmass
observations in the WFD area.
